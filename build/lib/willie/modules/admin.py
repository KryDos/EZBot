...
def part(bot, trigger):
    ...
    if not trigger.admin:
        return bot.reply("Piss on your grave!")
...
def invite_join(bot, trigger):
    """
    Join a channel ezbot is invited to, if the inviter is an admin.
    """
...
def hold_ground(bot, trigger):
    """
    This function monitors all kicks across all channels ezbot is in. If it
    detects that it is the one kicked it'll automatically join that channel.

    WARNING: This may not be needed and could cause problems if ezbot becomes
...
def mode(bot, trigger):
    """Set a user mode on ezbot. Can only be done in privmsg by an admin."""
    if not trigger.is_privmsg:
        return
    if not trigger.admin:
        return
...
def set_config(bot, trigger):
    """See and modify values of ezbots config object.
...
def save_config(bot, trigger):
    """Save state of ezbots config object to the configuration file."""
    if not trigger.is_privmsg:
        return
    if not trigger.admin:
        return
